...
app.post("/score", async (request, response) => {
  ...
  // receive and log the score data from the
  let score = request.body;
	console.log("Inserting score: " + JSON.stringify(score));

  // connect to the database and insert the score data
  let db = await MongoClient.connect(url);
  let result = await db.db("mydb")
  .collection("scores").insertOne(score);

  // log the the result and send response to client
  console.log("Inserted: " + JSON.stringify(score))
  response.json(score);
