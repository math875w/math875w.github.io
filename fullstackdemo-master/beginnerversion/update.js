...
// lazer pos
let LazerYPosition = birdYPosition;
// for each lazer
for(let Lazer of Lazers) {
    // draw the lazer
    drawImage(LazerImage,
        Lazer.xPosition,
        Lazer.yPosition,
        LazerImage.width,
        LazerImage.height
    ...
    if(Lazer.timer > LazerDespawnTimer){
        Lazers = Lazers.remove(Lazer);
        LazerDespawn = 0;
        LazerSound.play();
        ShootLazer = false;
        }
...
// spawn Lazer
if(gameState == "action" && ShootLazer == true) {
Lazers.push({
    timer: LazerDespawn,
    xPosition: LazerXPosition,
    yPosition: LazerYPosition
});
LazerDespawn = 0;
}
    ...
    for(let coin of coins) {
        ...
        //Check if Lazer collide with coin
        if(gameState == "action" && ShootLazer == true){
            for (let i = 300; i<1800; i += 100) {
                if(theseCirclesCollide(
                    i,
                    LazerYPosition,
                    LazerHitboxRadius,
    ...
    for(let fireball of fireballs) {
        ...
        // move the fireball
        fireball.xPosition += fireball.speed;
        ...
        //check if lazer collide with fireball
        if(gameState == "action" && ShootLazer == true){
            for (let i = 300; i<1800; i += 100) {
                console.log(i);
                if(theseCirclesCollide(
    ...
    if(gameState == "gameover" & highscoresingle == 0) {
