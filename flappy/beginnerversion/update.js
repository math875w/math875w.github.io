...
console.log(cloudTimeSinceLastSpawn);

    fillCanvas("rgb(135, 57, 104)");
...
// kode til update.js
    ...
    for(let house of houses) {
        ...
        house.xPosition += houseXSpeed;
        // remove cloud if it moves beyond the destruction point
        if(house.xPosition < destructionXPosition) {
            houses = houses.remove(house);
        }

    }
    // spawn a new cloud when the it is time
    houseTimeSinceLastSpawn += timeSinceLastFrame;
    if(houseTimeSinceLastSpawn>houseSpawnInterval) {
        houses.push({
            xPosition: spawnXPosition,
            yPosition: 700
            
        });
        houseTimeSinceLastSpawn = 0;
    }    
...
// for every cloud
 ...
 //rainbow pos
 let rainbowXPosition = birdXPosition-20;
 let rainbowYPosition = birdYPosition;
    // for each rainbow
    for(let rainbow of rainbows) {
     // draw the rainbow
     drawImage(rainbowImage,
         rainbow.xPosition,
         rainbow.yPosition,
         rainbowImage.width,
         rainbowImage.height
     ...
     // move the rainbow
     rainbow.xPosition += rainbowXSpeed;

      // remove rainbow if it goes off the screen
      if(rainbow.xPosition < destructionXPosition) {
         rainbows = rainbows.remove(rainbow);
     }
 }

 // spawn new rainbows
 if(gameState == "action" &&
 rainbowTimeSinceLastSpawn>rainbowSpawnInterval) {
     rainbows.push({
         xPosition: rainbowXPosition,
         yPosition: rainbowYPosition
     });
     rainbowTimeSinceLastSpawn = 0;
 }

 if(gameState == "action") {
     rainbowTimeSinceLastSpawn += timeSinceLastFrame;
 }
    ...
    birdTimer += timeSinceLastFrame;
    if(birdTimer>birdTimerInterval){
